...
struct Printer< ::cartesian_control_msgs::CartesianPosture_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::cartesian_control_msgs::CartesianPosture_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "posture_joint_names: ";
    if (v.posture_joint_names.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.posture_joint_names.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.posture_joint_names[i]);
    }
    if (v.posture_joint_names.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "posture_joint_values: ";
    if (v.posture_joint_values.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.posture_joint_values.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.posture_joint_values[i]);
    }
    if (v.posture_joint_values.empty() || true)
      s << "]";
  }
